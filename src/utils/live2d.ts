...
class Live2d {
  ...
  private initApp(view?: HTMLCanvasElement) {
    const canvas = view ?? document.getElementById('live2dCanvas') as HTMLCanvasElement

    if (this.app && this.canvas === canvas) return
  ...
  public resizeModel(modelSize: ModelSize, options: ResizeModelOptions = {}) {
    if (!this.model) return
    ...
    const scaleX = fitWidth / width
    const scaleY = fitHeight / height
    const scale = Math.min(scaleX, scaleY) * scaleMultiplier

    this.model.scale.set(scale)
    this.model.x = centerX
    this.model.y = centerY
